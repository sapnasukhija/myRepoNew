...
namespace CakeShop.Controllers
{
    ...
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Instructions([FromForm]OrderDto orderDto)
        {
            if (!ModelState.IsValid)
            {
                return View(orderDto);
            ...
            if (cartItems?.Count() <= 0)
            {
                ModelState.AddModelError("", "Your Cart is empty. Please add some cakes before Giving Intsructions");
                return View(orderDto);
            }
